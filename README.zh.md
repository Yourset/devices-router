...
Devices Router 只转发键盘，不转发鼠标。`v0.2.0` 固定了桌面控制台布局：周期状态刷新不再重建整页，只有日志查看区允许滚动。
...
- 连接端口：
  - TCP `8765`：键盘、可靠控制、心跳和 RTT 探测（已启用 `TCP_NODELAY`）
  - UDP `8766`：主电脑自动发现和鼠标活动快速上报
  - TCP `8767`：局域网自动更新
...
- 主电脑和副电脑自动发现、自动重连
- 鼠标活动自动切换键盘目标
- UDP 活动快速通道失败时自动回退到 TCP，不影响键盘连接
- 主电脑统一测量实时 RTT、稳定 RTT、抖动和近 20 次丢包
- 跨电脑鼠标移动、点击和滚轮转发保持停用
- 本机紧急释放快捷键和托盘安全按钮
- 主电脑/副电脑双向心跳检测连接状态
- 副电脑从主电脑局域网自动更新
...
- 主电脑现在可同时保持两台副电脑在线，第三台会被明确拒绝，不会挤掉现有连接。
- 鼠标继续由 Logi Flow 处理；Devices Router 只观察三台电脑的鼠标活动，并把键盘切到最后活动的电脑。
- 主电脑界面会显示本机和两个副电脑槽位，可按设备 ID 手动切换并设置别名。
- 安全键仍是主电脑本地的 `Ctrl+Alt+Esc`；当前目标断线或发送失败时立即回到主电脑。
- 升级顺序：先升级主电脑，再升级两台副电脑。新旧混用期间只允许一台没有设备 ID 的 legacy 副电脑。
