...
一个让一套普通键盘鼠标控制两台 Windows 电脑的小工具。

Devices Router 可以直接转发键盘和鼠标，不依赖 Logitech Flow 或罗技硬件。目标是尽量无脑：两台电脑都打开软件，从主电脑右边缘划出，键盘鼠标就一起跟到副电脑。
...
6. 确认两边的游戏模式均已关闭；`v0.1.27` 默认开启跨电脑鼠标控制。
7. 从主电脑右边缘向右划，鼠标和键盘应进入副电脑；把副电脑光标推到左边缘后继续向左即可返回。
8. 任何时候都可以在主电脑按 `Ctrl+Alt+Esc`，或点击托盘里的 `立即释放控制`，强制恢复本机键盘鼠标。
...
- 主电脑安全键 `Ctrl+Alt+Esc`：立即释放键盘和鼠标，不依赖副电脑或网络
