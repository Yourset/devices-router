...
Since `v0.1.19`, Devices Router sends local modifier key-up events before suppressing host input for remote control. If it still happens:

1. Press and release `Ctrl`, `Alt`, `Shift`, and `Win` once to reset Windows state.
2. Confirm both sides are on `v0.1.19` or newer.
...
Since `v0.1.19`, arrow keys, Home/End, PageUp/PageDown, Insert/Delete, and related navigation keys use scan-code based extended-key injection. If it still fails:

1. Confirm the remote PC is on `v0.1.19` or newer.
